...
export class AuthController {
  ...
  public async signup (req: Request, res: Response): Promise<any> {
    try {
      ...
      // Check if the service exists and is public
      const existingServices = await this.servicesService.findByNames(service)
      ...
      // if the user exists, check if the user has access to the service
      if (existingUserByEmail) {
        ...
        if (userHasService) {
          return res.status(409).json({
            success: false,
            message: 'Conflict, You already have access to this service',
            error: `Email is already taken with ${service} service`
          })
        }
        ...
        return res.status(201).json({
          success: true,
          message: `User signed up successfully with ${service} service`,
          data: {
            user: {
  ...
  public async login (req: Request, res: Response): Promise<any> {
    try {
      ...
      return res.json({
        success: true,
        message: `Login successfull with ${service} service`,
        data: {
          user: {
            id: user.id,
            email: user.email,
